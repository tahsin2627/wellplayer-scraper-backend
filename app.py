...
def scrape_sflix(tmdb_id, media_type, season=None, episode=None):
    ...
    try:
        ...
        episodes_response = requests.get(episodes_api_url, headers=HEADERS, timeout=12)
        ...
        if not episode_id:
            print(f"SFlix: Could not find episode ID for {tmdb_id} S{season}E{episode}")
            return []

        servers_api_url = f"{base_url}/ajax/episode/servers/{episode_id}"
        servers_response = requests.get(servers_api_url, headers=HEADERS, timeout=12)
        ...
        for server_item in servers_soup.select('.server-item'):
            ...
            final_link_response = requests.get(final_link_api_url, headers=HEADERS, timeout=12)
...
def scrape_vidsrc_to_sources(tmdb_id, media_type, season=None, episode=None):
    ...
    try:
        ...
        for server_link in server_divs.find_all('li'):
            ...
            source_response = requests.get(source_url, headers={'Referer': embed_url, 'User-Agent': USER_AGENT}, timeout=12)
...
@app.route('/')
def index():
    return "WellPlayer Scraper Backend (Stable Two-Source Edition) is running!"
...
def get_movie_details(tmdb_id):
    ...
    if not all_links:
        print("SFlix failed for movie, trying VidSrc.to as backup...")
        all_links = scrape_vidsrc_to_sources(tmdb_id, 'movie')
...
def get_episode_links():
    ...
    if not all_links:
        print(f"SFlix failed for S{season_num}E{ep_num}, trying VidSrc.to as backup...")
        all_links = scrape_vidsrc_to_sources(tmdb_id, 'tv', season_num, ep_num)
